...
function parseAtributos(nombre = "", categoria = "") {
  ...
  // --- CHAPAS / TECHOS ---
  if (c.startsWith("techos") || /\bchapa(s)?\b/.test(t)) {
    attrs.familia = "chapas";

    // perfil
    if (/\bt101\b/.test(t) || /\btrapezoidal\b/.test(t)) attrs.perfil = "trapezoidal";
    else if (/\bsinusoidal\b/.test(t) || /\bondulada\b/.test(t)) attrs.perfil = "sinusoidal";
    else if (/\blisa\b/.test(t)) attrs.perfil = "lisa";

    // terminación/material
    if (/\bcincalum\b|\baluzinc\b|\bzincalum\b/.test(t)) attrs.terminacion = "cincalum";
    else if (/\bgalvanizad/.test(t)) attrs.terminacion = "galvanizada";
    else if (/\bprepintad/.test(t) || /\bcolor\b/.test(t)) attrs.terminacion = "prepintada";

    // traslúcidas
    if (/\bpolicarbonato\b/.test(t)) attrs.tipo = "policarbonato";
    else if (/\btraslucid/.test(t) || /\bplast/.test(t)) attrs.tipo = "traslucida";
    if (/\bclaraboy/.test(t)) attrs.extra = "claraboya";
    ...
    if (mm) attrs.espesor_mm = Number(mm[1]);
    const cal = t.match(/\b(?:nro|no|c-)\s*(\d{2})\b/);
    if (cal) attrs.calibre = cal[1];
  }

  // --- CERÁMICAS / PORCELANATOS ---
  if (/\bporcelanat\b|\bceramic\b|\brevest\b|\bpiso\b/.test(c + " " + t)) {
    attrs.familia = attrs.familia || "ceramicos";

    const dim = t.match(/\b(\d{2,3})\s*x\s*(\d{2,3})\b/);
    if (dim) attrs.medida_cm = `${dim[1]}x${dim[2]}`;

    if (/\bexterior\b/.test(t) || /\bext\b/.test(t)) attrs.uso = "exterior";
    else if (/\binterior\b/.test(t) || /\bint\b/.test(t)) attrs.uso = "interior";

    if (/\bantidesliz\b/.test(t)) attrs.acabado = "antideslizante";
    else if (/\brectificad/.test(t)) attrs.acabado = "rectificado";
    else if (/\bpulid/.test(t)) attrs.acabado = "pulido";
    else if (/\bsatinad/.test(t)) attrs.acabado = "satinado";
  }

  // --- PINTURAS / MEMBRANAS ---
  if (/\bpintur\b|\blatex\b|\besmalt\b|\bmembran\b|\bimpermeab\b/.test(c + " " + t)) {
    attrs.familia = attrs.familia || "pinturas";

    const l = t.match(/\b(\d+(?:\.\d+)?)\s*(l|lt|lts|litro|litros)\b/);
    if (l) attrs.litros = Number(l[1]);

    if (/\binterior\b/.test(t)) attrs.uso = "interior";
    else if (/\bexterior\b/.test(t)) attrs.uso = "exterior";
  }

  // --- HIERROS ---
  if (/\bhierro\b|\bvarilla\b|\bfierro\b|\bbarra\b|\bmalla\b|\bperfil\b/.test(c + " " + t)) {
    attrs.familia = attrs.familia || "hierros";
    const d = t.match(/\b(\d+(?:\.\d+)?)\s*mm\b/);
    if (d) attrs.diametro_mm = Number(d[1]);
  }
    ...
    const productosLimpios = catalogoCompleto.productos
      ...
      .map((p) => {
        ...
        return {
          ...
          atributos: parseAtributos(p.nombre, mejorCategoria),
        };
      });
